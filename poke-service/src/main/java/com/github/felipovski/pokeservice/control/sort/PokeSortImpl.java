...
@Component
public class PokeSortImpl implements PokeSort {
     ...
     */
    @Override
    public List<PokemonDto> sort(List<PokemonDto> pokemons, SortType sortType) {
    ...
    @Override
    public List<PokemonDto> merge(List<PokemonDto> left, List<PokemonDto> right, SortType sortType) {
        ...
        while (a < left.size() && b < right.size()) {
            boolean addLeft;
            addLeft = switch (sortType) {
                // If sortType is ALPHABETICAL, then compare the strings
                case ALPHABETICAL -> left.get(a).getName().compareToIgnoreCase(right.get(b).getName()) < 0;
                // If sortType is LENGTH, then compare name lengths
                case LENGTH -> left.get(a).getName().length() < right.get(b).getName().length();
            };
